...
db = client["Chatbott1"]  # Database name
...
with st.sidebar:
    ...
    # Display session-based history
    for session, history in st.session_state.chat_history.items():
        st.subheader(f"Session {session}")
        for msg in history:
    ...
    if st.button("Store messages in CH"):
        session_id = st.session_state.session_count
